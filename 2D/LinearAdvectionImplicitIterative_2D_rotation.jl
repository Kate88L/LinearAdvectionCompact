...
ghost_point_right = zeros(N + 2, Ntau + 2);
ghost_point_top = zeros(N + 2, Ntau + 2);

# Boundary condition
for n = 2 : Ntau + 2
    phi[1, :, n] = phi_exact.(xL, y, t[n]);
    phi[:, 1, n] = phi_exact.(x, yL, t[n] );
    phi[end, :, n] = phi_exact.(xR, y, t[n]);
    phi[:, end, n] = phi_exact.(x, yR, t[n]);
    phi_star[1, :, n] = phi_exact.(xL, y, t_star[n]);
    phi_star[:, 1, n] = phi_exact.(x, yL, t_star[n]);
    phi_star[end, :, n] = phi_exact.(xR, y, t_star[n]);
    phi_star[:, end, n] = phi_exact.(x, yR, t_star[n]);
    phi_first_order[1, :, n] = phi_exact.(xL, y, t[n]);
    phi_first_order[:, 1, n] = phi_exact.(x, yL, t[n] );
    phi_first_order[end, :, n] = phi_exact.(xR, y, t[n]);
    phi_first_order[:, end, n] = phi_exact.(x, yR, t[n]);

    # Ghost point on the inflow boundary
    ghost_point_left[:, n] = phi_exact.(xL - h, y, t[n]);
    ghost_point_bottom[:, n] = phi_exact.(x, yL - h, t[n]);
    ghost_point_right[:, n] = phi_exact.(xR + h, y, t[n]);
    ghost_point_top[:, n] = phi_exact.(x, yR + h, t[n]);
end

phi_star[1, :, end] = phi_exact.(xL, y, t_star[end]);
phi_star[:, 1, end] = phi_exact.(x, yL, t_star[end]);
phi_star[end, :, end] = phi_exact.(xR, y, t_star[end]);
phi_star[:, end, end] = phi_exact.(x, yR, t_star[end]);
...
for n = 1 : Ntau
    ...
    for i = N + 1 : -1 : 1
        ...
        for j = 2 : N + 2
            ...
            phi_first_order[i, j, n + 1] = ( phi_first_order[i, j, n] + abs(c[i, j]) * (c[i,j] < 0) * phi_first_order[i + 1, j, n + 1] 
                                                                      + abs(d[i, j]) * (d[i,j] > 0) * phi_first_order[i, j - 1, n + 1]) / (1 + abs(c[i, j]) + abs(d[i, j]) );
...
trace2 = contour(x = x, y = y, z = phi_exact.(X, Y, 0), name = "Implicit", showscale=false, colorscale = "Plasma", contours_coloring="lines", line_width=1)
